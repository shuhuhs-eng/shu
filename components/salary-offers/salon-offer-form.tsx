...
export function SalonOfferForm({ stylistUserId, existingOffer }: { stylistUserId: string; existingOffer: Offer | null }) {
  ...
  const [guarantee, setGuarantee] = useState(250000); const [addition, setAddition] = useState(0); const [months, setMonths] = useState<1|3|6|12>(3); const [note, setNote] = useState("");
  ...
  async function submit() { setBusy(true); setMessage(null); const result = await createSalaryOffer({ stylistUserId, monthlyGuarantee: guarantee, performanceAddition: addition, guaranteeMonths: months, message: note || null }); setBusy(false); setMessage(result.success ? "条件を提示しました。" : result.error); }
  return <div className="mt-3 rounded-xl border border-line bg-surface p-4">
    <p className="text-[12px] font-bold text-ink">給与条件を提示</p><p className="mt-1 text-[10.5px] text-sub">入力は3項目だけです。雇用契約の確定ではありません。</p>
    ...
    <div className="mt-3 flex gap-2"><button type="button" onClick={()=>setOpen(false)} className="flex-1 rounded-full border border-line py-2 text-[11px]">閉じる</button><button type="button" disabled={busy} onClick={submit} className="flex-1 rounded-full bg-ink py-2 text-[11px] font-bold text-surface disabled:opacity-50">{busy?"送信中...":"提示する"}</button></div>
